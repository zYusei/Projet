...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;font-family:'Plus Jakarta Sans',sans-serif;}
    body{
      min-height:100vh;background:linear-gradient(135deg,var(--bleu-vif),var(--orange-vif));
    ...
    /* CARD */
    .login-container{
      background:var(--blanc);border-radius:20px;box-shadow:0 20px 30px var(--shadow-light),0 0 0 1px rgba(0,0,0,.05);
    ...
    /* >>> FOCUS BLEU pour text/email/password */
    input[type="text"],
    input[type="email"],
    input[type="password"]{
      width:100%;padding:.85rem 1rem .85rem 45px;border:2.5px solid var(--input-border);border-radius:12px;
      font-size:1rem;background:var(--input-bg);color:var(--gris-fonce);
      transition:border-color .4s, box-shadow .4s;outline-offset:3px;
    }
    input[type="text"]:focus,
    input[type="email"]:focus,
    input[type="password"]:focus{
      border-color:var(--bleu-vif);box-shadow:0 0 8px var(--bleu-vif);background:#e9f0ff;
    }
    input[type="text"]:focus + svg,
    input[type="email"]:focus + svg,
    input[type="password"]:focus + svg{fill:var(--bleu-vif);}

    .error-message{font-size:.85rem;color:#dc3545;margin-top:4px;font-weight:600;user-select:none;min-height:1.2em;}

    .btn-submit{
      position:relative;overflow:hidden;background:var(--orange-vif);color:#fff;font-weight:700;padding:.85rem 0;border:none;border-radius:14px;
      cursor:pointer;font-size:1.25rem;box-shadow:0 8px 15px rgba(255,165,0,.4);transition:background-color .35s, box-shadow .35s;user-select:none;
    }
...
<script>
  ...
  // Nettoyage message d’erreur à la saisie
  ...
  // Force du mot de passe (barre + texte Faible/Moyen/Fort)
